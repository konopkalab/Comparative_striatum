...
# Save (optional)
write_rds(seurM_corr, '~/workdir/01_MARMOSET/MARMOSET_KRIENEN/03_CELLBENDERED_ANNOTATE/PUTAMEN/KRIENEN_PUTAMEN_CLUSTERING1.RDS')

# Move the plots
outfls = list.files('~/', pattern = pref, full.names = F)
todir = '~/workdir/01_MARMOSET/MARMOSET_KRIENEN/03_CELLBENDERED_ANNOTATE/PUTAMEN/CLUSTERING1/'
dir.create(todir)
...
# Move the plots
outfls = list.files('~/', pattern = pref, full.names = F)
todir = '~/workdir/01_MARMOSET/MARMOSET_KRIENEN/03_CELLBENDERED_ANNOTATE/PUTAMEN/CLUSTERING2/'
dir.create(todir)
sapply(outfls, function(x){file.copy(from = paste0('~/', x), to = paste0(todir, x))})
file.remove(outfls)
...
mapnames = setNames(c('MOL', 'SPN', 'Astrocyte', 'SPN', 'SPN', 'SPN',
			'Microglia', 'SPN', 'SPN', 'OPC', 'Astrocyte',
			'Non_SPN', 'MOL', 'MOL', 'Non_SPN', 'Non_SPN',
			'Non_SPN', 'SPN', 'Non_SPN'),
		      c(0:18))
