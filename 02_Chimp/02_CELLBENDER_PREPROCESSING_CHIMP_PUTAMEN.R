...
caudSamps = allsamps[allsamps$Brain_Region == 'Putamen' & allsamps$Species == 'Chimp',]
...
fls = fls[grepl(paste0(rownames(caudSamps), collapse = '|'), fls)]
...
write_rds(allseur, '~/workdir/01_CHIMP/02_CELLBENDERED_CLUSTER/Chimp_Putamen_Seurat_CB_Raw.RDS')
allseur = read_rds('~/workdir/01_CHIMP/02_CELLBENDERED_CLUSTER/Chimp_Putamen_Seurat_CB_Raw.RDS')

# Update matrix if the initial filtering was done incorrectly
#allseur_orig = read_rds('~/workdir/01_CHIMP/02_CELLBENDERED_CLUSTER/Chimp_Putamen_Seurat_CB_Raw.RDS')
...
#write_rds(newSeur, '~/workdir/01_CHIMP/02_CELLBENDERED_CLUSTER/Chimp_Putamen_Seurat_CB_Raw.RDS')
...
# I normally do not implement this filter, but CellBender filtering was very weak dataset.
...
# Save (optional)
write_rds(seurM_corr, paste0('~/workdir/01_CHIMP/02_CELLBENDERED_CLUSTER/', pref, '_Clustering1.RDS'))
seurM_corr = read_rds(paste0('~/workdir/01_CHIMP/02_CELLBENDERED_CLUSTER/', pref, '_Clustering1.RDS'))

qw = FindMarkers(seurM_corr, ident.1 = 9, only.pos = T, min.pct = 0.5, logfc.threshold = 0.5)
...
seurM_corr = clusterBatchCorrect(seurM, npcs = 30, res = 0.5, pref = 'Chimp_Putamen', sampleName = 'orig.ident')
...
write_rds(seurM_corr, paste0('~/workdir/01_CHIMP/02_CELLBENDERED_CLUSTER/', pref, '_Clustering2.RDS'))

qw = FindMarkers(seurM_corr, ident.1 = 15, only.pos = T, min.pct = 0.5, logfc.threshold = 0.5)
DotPlot(seurM_corr, features = rownames(qw)[1:30]) + rotate_x_text(90)
...
write_rds(seurM_corr, paste0('~/workdir/01_CHIMP/02_CELLBENDERED_CLUSTER/', pref, '_Clustering3.RDS'))
...
# Also remove sample 7 since it differs from other samples. It as low number of SPNs for some reason.
vasculature = c()
empty_drop = c()
doublet = c(17,23)
seurM = subset(seurM_corr, subset = seurat_clusters %in% c(vasculature, empty_drop, doublet), invert = T)
...
# Save (optional)
dir.create('~/workdir/01_CHIMP/02_CELLBENDERED_CLUSTER/CLUSTERING_4')
write_rds(seurM_corr, paste0('~/workdir/01_CHIMP/02_CELLBENDERED_CLUSTER/', pref, '_Clustering4.RDS'))
...
seurM_corr = read_rds(paste0('~/workdir/CHIMP/02_CELLBENDERED_CLUSTER/', pref, '_Clustering4.RDS'))

mapnames = setNames(c('MOL', 'SPN', 'MOL', 'MOL', 'Microglia', 'Astrocyte',
			'MOL', 'OPC', 'MOL', 'MOL', 'Astrocyte',
			'SPN', 'SPN', 'SPN', 'Other_GABA', 'Other_GABA',
			'MOL', 'Other_GABA', 'Other_GABA', 'Other_GABA', 'MOL',
			'COP', 'MOL'),
		      c(0:22))
...
# Save
stackedbarplot(seurM_corr[[]], groupfill = 'newannot', groupx = 'orig.ident', fn = 'Chimp_Stacked_Putamen_Annotated')
write_rds(seurM_corr, paste0('~/workdir/01_CHIMP/02_CELLBENDERED_CLUSTER/', pref, '_Annotated_FINAL.RDS'))
